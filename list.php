...
if($mode=="search"){
	...
	if($ctgs=="notice.html")
	{
		$sql="select * from dnjstjq25.greet where $find like '%search%' and ctg='notice' order by num desc";
	}
	elseif($ctgs=="news.html")
	{
		$sql="select * from dnjstjq25.greet where $find like '%search%' and ctg='news' order by num desc";
	}
	elseif ($ctgs=="service.html")
	{
		$sql="select * from dnjstjq25.greet where $find like '%search%' and ctg='service' order by num desc";
	}
} else {
	if($ctgs=="notice.html")
	{
		$sql="select * from dnjstjq25.greet where ctg='notice' order by num desc limit $first_num, $scale";
	}
	elseif($ctgs=="news.html")
	{
	...
	elseif($ctgs=="service.html")
	{
		$sql="select * from dnjstjq25.greet where ctg='service' order by num desc limit $first_num, $scale";
	}	
}
...
try{
	...
	if($ctgs=="notice.html")
	{
		$sql = "select * from dnjstjq25.greet where ctg='notice'";
	}
	elseif($ctgs=="news.html")
	{
	...
	elseif ($ctgs=="service.html") {
		$sql = "select * from dnjstjq25.greet where ctg='service'";
	}
	$stmh1 = $pdo->query($sql);
	...
	?>
<body>
<form name="board_form" method="post" action="notice.html?mode=search">
	<div id="list_search">
...
while($row = $stmh->fetch(PDO::FETCH_ASSOC)){
	...
	<div id="list_item">
		...
		<div id="list_item2"><a href="view.html?num=<?=$item_num?>&page=<?=page?>"><?=$item_subject?></a></div>
